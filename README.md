...
- Frontend で任意のログイン/ログアウト導線を表示できます
  ...
  - `VITE_APP_VERSION`（フッター表示用バージョン。未設定時は `v0.3.5-next`）
- 注意:
...
## Backup (pg_dump)
- Automatic backup runs in the `backup` service.
- Output destination is controlled by `.env`: `BACKUP_OUTPUT_DIR` (default: `./backups`).
- Interval and retention are also `.env` configurable:
  - `BACKUP_INTERVAL_SECONDS` (default: 86400)
  - `BACKUP_RETENTION_DAYS` (default: 14)
  - `BACKUP_FILE_PREFIX` (default: syukatsu)
